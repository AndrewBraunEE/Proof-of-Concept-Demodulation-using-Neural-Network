...
def run():
	...
	try:
		...
		if args.train:
			...
			s = NND("./tf.model",5, 128, 64, 32, 0.001, decoded_waveform = app_encoder.decoded_binary_pulse(binary_str_unencoded), ErrorObject = ErrorObject, batch_size = app_encoder.get_modulator_default().tb, waveform = waveform_samples, freq = encoder_app.freq)
